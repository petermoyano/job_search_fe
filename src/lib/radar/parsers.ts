...
function parseQualityReviewStatus(value: unknown, path: string): QualityReviewStatus {
  switch (value) {
    ...
    case "completed":
      return value;
    default:
      throw new RadarContractError(`${path} contiene un estado de revisi�n desconocido.`);
  }
}
...
function parseQualityReviewVerdict(
  ...
  throw new RadarContractError(`${path} contiene un veredicto de revisi�n desconocido.`);
}
